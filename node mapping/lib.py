    ...
    for i in range(len(listTree)):
        ...
        for rel in rels:
            ...
            if len(tree.search_nodes(word = rel)) > 0:
                for j in range(len(tree.search_nodes(word = rel))):
                    tempTree = tree.copy()
                    predicateNode = tempTree.search_nodes(word = rel)[j]
                    predicateNode.add_features(predicate = True)
                    for node in tempTree.traverse("postorder"):
                        if (isPhraseType(node.name) or isSType(node.name)) and (node.up != tempTree) and not (node in predicateNode.get_ancestors()):
                            collect(node)
                newListID.append(id)
                newListTree.append(tempTree)
                newListRel.append(rel)
                ...
                tempTree = tree.copy()
                predicateNode = getLeavesPredicate(tempTree, rel, id)
                # print tempTree.get_ascii()
                # for leaf in tempTree:
                #   print leaf.word
                if predicateNode == None:
                    #print id
                    #print rel
                    continue
                predicateNode.add_features(predicate = True)
...
def getFeature(listID, listTree, listRel, listArg, listWordName, listCluster):
    ...
    for i in range(len(listTree)):
        ...
        cnt = 0
        constituents = getContituentNodes(tree)
...
def evaluationSVM1Stage(listFeatureTrain, listLabelTrain, listFeatureTest, listLabelTest, numArg, listFeatureName, listEncode):
    ...
    listLabelTrain = np.asarray(listLabelTrain)
    listLabelTest = np.asarray(listLabelTest)
    listLabelPredict = classificationSVM(listFeatureTrain, listLabelTrain, listFeatureTest)
    precisionChunking, recallChunking, f1ScoreChunking = getF1ScoreChunking(listLabelTest, numArg, listEncode)
    precisionClassify1Stage, recallClassify1Stage, f1ScoreClassify1Stage = getF1ScoreClassify1Stage(listLabelTest, listLabelPredict, listEncode)
    precision1Stage, recall1Stage, f1Score1Stage = getF1ScoreTotal(listLabelTest, listLabelPredict, numArg, listEncode)
    return precisionChunking, recallChunking, f1ScoreChunking, precision1Stage, precisionClassify1Stage, recallClassify1Stage, f1ScoreClassify1Stage, recall1Stage, f1Score1Stage
...
def crossValidation(groupListLabel, groupListFeature, listLE, leLabel, foldNumber, groupInfo):
    ...
    return listOfListFeatureTrain, listOfListFeatureTest, listOfListLabelTrain, listOfListLabelTest, listOfListNumArg
...
def crossValidationSVM1Stage(listOfListFeatureTrain, listOfListFeatureTest, listOfListLabelTrain, listOfListLabelTest, listFeatureName, foldNumber, listOfListNumArg, listEncode):
    ...
    for i in range(foldNumber):
        ...
        listLabelTest = listOfListLabelTest[i]
        numArg = listOfListNumArg[i]
        precisionChunking, recallChunking, f1ScoreChunking, precision1Stage, precisionClassify1Stage, recallClassify1Stage, f1ScoreClassify1Stage, recall1Stage, f1Score1Stage = evaluationSVM1Stage(listFeatureTrain, listLabelTrain, listFeatureTest, listLabelTest, numArg, listFeatureName, listEncode)
        listPrecisionChunking.append(precisionChunking)
        listRecallChunking.append(recallChunking)
        listF1ScoreChunking.append(f1ScoreChunking)
        listPrecisionClassify1Stage.append(precisionClassify1Stage)
...
def output1Stage(listPrecisionChunking, listRecallChunking, listF1ScoreChunking, listPrecisionClassify1Stage, listRecallClassify1Stage, listF1ScoreClassify1Stage, listPrecision1Stage, listRecall1Stage, listF1Score1Stage):
...
def kFold(listID, listTree, listRel, listArg, listWordName, listCluster, numberGroup, listLabelReduce):
    ...
    return groupInfo, groupListLabel, groupListFeature
...
def omitUnderscore(listID, listTree, listRel, listArg):
    """omit underscore after chunking
    """
    for i in range(len(listID)):
        for leaf in listTree[i]:
